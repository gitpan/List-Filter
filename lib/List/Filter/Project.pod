...
system is very (perhaps, "pathlogically") extensible in various
ways.
...
which allows the usage of any storage backend with a DBD driver.
...
the method in that moodule's @EXPORT array.
...
and passing them on seperately), but after that the "method"
...
which these methods can use to share resources.
...
The names of the plug-in's object data has an obvious DBI bias,
but these may be used in any way that seems appropriate:
"connect_to", "owner", "password", and "attributes".

In the case of YAML, "connect_to" is assigned the path to a yaml
file, and "owner" and "password" are un-used.

There is also the "extra" hashref, which can be used for
additional miscellanious pieces of information.
...
This is a hashref of hashrefs, which might be thought of as a
...
There is an analogus collection of 'transform" libraries:
...
This project is based on L<Class::Base> which provides a simple "new"
routine, that calls an initialization routine called "init" (note: no
leading underscore).  The purpose of this seperation is so that a
child class can have an "init" that invokes "SUPER::init", combining
the init routines of both parent and child.

Note: there is no leading underscore on "init", though it's
arguably an "internal" routine.
...
automatically generated from templates: it's kept at the bottom of
the module files because it typically isn't very interesting.

The accessor naming convention: setters are named with the traditional
"set_" prepended to the field name, but getters just use the field
name without any prefix ("mutators" are not used, because they don't
allow setting a value to undef).  Thus the most common case gets the
simplest syntax:
...
Inheritance has been used in several places as a convienient way
...
A number of parts in this design, shall we say, push the limits
of good taste in object-oriented design: for example, rather than
implement an object class for each type of filter (so that they
each know how to apply themselves) a less rigid approach has been
taken where each filter knows a default "method" of application,
but can be used in other ways.

There are also some funky code-smells in the storage system,
where both "filters" and the variant "transforms" can be saved
and retrieved using the same code, which typically determines
which type it's saving and retrieving via naming conventions.
...
code for doing the simplest, most common thing (using yaml
files): So instead of, for example, defining an array of storage
objects, one defines a list of heterogenous items that the
storage handler must then try to make sense of.  Internally, this
does become an array of storage objects. While access to these
objects is available, it is not expected that the client coder
will often need get at them directly.
